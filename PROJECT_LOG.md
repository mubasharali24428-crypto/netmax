...
- **Test-suite agent**: offline pytest suite per TDD/systematic-debugging skills. **Done** — 28 offline tests passing (`tests/test_netmax.py`, network fully mocked; run via `/Users/user/1/bin/python -m pytest`).
- **GUI agent**: Tkinter app per TDD/systematic-debugging skills. **Done** — `netmax_gui.py` + 20 headless GUI tests passing (`tests/test_netmax_gui.py`; run via `/Users/user/1/bin/python -m pytest`).
...
```bash
cd ~/netmax
/Users/user/1/bin/python netmax.py full --seconds 10     # CLI report
/Users/user/1/bin/python netmax_gui.py                   # desktop GUI
/Users/user/1/bin/python measure.py                      # regenerate charts
```

Interpreter note: use `/Users/user/1/bin/python` (has matplotlib/Tkinter);
plain `python3` is the Hermes venv and lacks them.
...
- Both suites verified via `/Users/user/1/bin/python -m pytest`.
